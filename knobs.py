...
class Knob:
    ...
    def __init__(self, knobs, internal_metrics, external_metrics, opt):
        ...
        ''' default 파일 얻은 다음 get_trg_default활성화'''
    ...
    def split_data(self): # s_wk is similar workload with target workload
        if self.opt.train_type == 'wmaml':
            self.X_tr_dict, self.X_val_dict, self.X_te_dict, self.X_tmp_dict = {}, {}, {}, {}
            self.im_tr_dict, self.im_val_dict, self.im_te_dict, self.im_tmp_dict = {}, {}, {}, {}
            self.em_tr_dict, self.em_val_dict, self.em_te_dict, self.em_tmp_dict = {}, {}, {}, {}
            
            for i in range(len(self.internal_metrics)):
                self.X_tmp_dict[i], self.X_te_dict[i], self.im_tmp_dict[i], self.im_te_dict[i], self.em_tmp_dict[i], self.em_te_dict[i] = \
                    train_test_split(self.knobs, self.internal_metrics[i], self.external_metrics[i], test_size=0.2, random_state=22)

                self.X_tr_dict[i], self.X_val_dict[i], self.im_tr_dict[i], self.im_val_dict[i], self.em_tr_dict[i], self.em_val_dict[i] = \
                    train_test_split(self.X_tmp_dict[i], self.im_tmp_dict[i], self.em_tmp_dict[i], test_size=0.5, random_state=22)                
    ...
    def scale_data(self):
        if self.opt.train_type == 'wmaml':
            ...
            self.norm_X_tr_dict, self.norm_X_val_dict, self.norm_X_te_dict = {}, {}, {}
            self.norm_im_tr_dict, self.norm_im_val_dict, self.norm_im_te_dict = {}, {}, {} 
            self.norm_em_tr_dict, self.norm_em_val_dict, self.norm_em_te_dict = {}, {}, {}

            for i in range(len(self.internal_metrics)):
                # self.scaler_X_dict[i] = MinMaxScaler().fit(self.X_tr_dict[i])
                # self.scaler_im_dict[i] = MinMaxScaler().fit(self.im_tr_dict[i]) # [0, 1]
                # self.scaler_em_dict[i] = MinMaxScaler().fit(self.em_tr_dict[i])
                
                # self.norm_X_tr_dict[i] = torch.Tensor(self.scaler_X_dict[i].transform(self.X_tr_dict[i])).cuda()
                # self.norm_X_val_dict[i] = torch.Tensor(self.scaler_X_dict[i].transform(self.X_val_dict[i])).cuda()
                # self.norm_X_te_dict[i] = torch.Tensor(self.scaler_X_dict[i].transform(self.X_te_dict[i])).cuda()

                # self.norm_im_tr_dict[i] = torch.Tensor(self.scaler_im_dict[i].transform(self.im_tr_dict[i])).cuda()
                # self.norm_im_val_dict[i] = torch.Tensor(self.scaler_im_dict[i].transform(self.im_val_dict[i])).cuda()
                # self.norm_im_te_dict[i] = torch.Tensor(self.scaler_im_dict[i].transform(self.im_te_dict[i])).cuda()

                # self.norm_em_tr_dict[i] = torch.Tensor(self.scaler_em_dict[i].transform(self.em_tr_dict[i])).cuda()
                # self.norm_em_val_dict[i] = torch.Tensor(self.scaler_em_dict[i].transform(self.em_val_dict[i])).cuda()
                # self.norm_em_te_dict[i] = torch.Tensor(self.scaler_em_dict[i].transform(self.em_te_dict[i])).cuda()

                self.scaler_X_dict[i] = MinMaxScaler().fit(self.X_tr_dict[i])
                self.scaler_im_dict[i] = MinMaxScaler().fit(self.im_tr_dict[i]) # [0, 1]
                self.scaler_em_dict[i] = MinMaxScaler().fit(self.em_tr_dict[i])
                
                self.norm_X_tr_dict[i] = self.scaler_X_dict[i].transform(self.X_tr_dict[i])
                self.norm_X_val_dict[i] = self.scaler_X_dict[i].transform(self.X_val_dict[i])
                self.norm_X_te_dict[i] = self.scaler_X_dict[i].transform(self.X_te_dict[i])

                self.norm_im_tr_dict[i] = self.scaler_im_dict[i].transform(self.im_tr_dict[i])
                self.norm_im_val_dict[i] = self.scaler_im_dict[i].transform(self.im_val_dict[i])
                self.norm_im_te_dict[i] = self.scaler_im_dict[i].transform(self.im_te_dict[i])

                self.norm_em_tr_dict[i] = self.scaler_em_dict[i].transform(self.em_tr_dict[i])
                self.norm_em_val_dict[i] = self.scaler_em_dict[i].transform(self.em_val_dict[i])
                self.norm_em_te_dict[i] = self.scaler_em_dict[i].transform(self.em_te_dict[i])

                
            
            self.norm_X_dict, self.norm_im_dict, self.norm_em_dict = {}, {}, {}

            self.norm_X_dict['tr'] , self.norm_X_dict['val'], self.norm_X_dict['te'] = \
                self.norm_X_tr_dict, self.norm_X_val_dict, self.norm_X_te_dict

            self.norm_im_dict['tr'] , self.norm_im_dict['val'], self.norm_im_dict['te'] = \
                self.norm_im_tr_dict, self.norm_im_val_dict, self.norm_im_te_dict            

            self.norm_em_dict['tr'] , self.norm_em_dict['val'], self.norm_em_dict['te'] = \
                self.norm_em_tr_dict, self.norm_em_val_dict, self.norm_em_te_dict    

            # self.default_trg_em = self.scaler_em_dict[self.target_wk].transform(self.default_trg_em)
